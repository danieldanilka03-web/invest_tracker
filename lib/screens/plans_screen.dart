...
class _PlansScreenState extends State<PlansScreen> {
  ...
  List<Plan> _sorted(List<Plan> list) {
    return list..sort((a, b) => (a.targetDate ?? DateTime(2100)).compareTo(b.targetDate ?? DateTime(2100)));
  }

  @override
  Widget build(BuildContext context) {
    final all = StorageService.plans;
    final active = _sorted(all.where((p) => p.status == PlanStatus.active).toList());
    final done = _sorted(all.where((p) => p.status == PlanStatus.done).toList());
    final cancelled = _sorted(all.where((p) => p.status == PlanStatus.cancelled).toList());
